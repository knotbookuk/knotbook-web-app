...
const PLANS: PlanDef[] = [
  {
    ...
    features: [
      ...
      "Beauty professional management",
      "Export to CSV",
    ],
  },
  {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-[520px] rounded-3xl bg-surface-container-low animate-pulse" />
        ))}
      </div>
    </div>
  );
...
function SubscriptionContent() {
  ...
  // Filter plans based on user type — couples see Personal only, planners see Planner tiers
  const filteredPlans = PLANS.filter((plan) => {
    if (userType === "PLANNER") {
      return plan.tierKey === "planner_basic" || plan.tierKey === "planner_advanced";
    }
    return plan.tierKey === "couple";
  });
        ...
        {filteredPlans.map((plan) => {
          ...
          if (contextHasPlan) {
            if (isCurrentPlan) {
              ...
              const tierOrder = ["couple", "planner_basic", "planner_advanced"];
        ...
        <>
          ...
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-on-surface-variant font-label max-w-lg mx-auto">
              Unlock every feature to plan your perfect wedding day. Pick the plan that suits you best.
            </p>
          </div>
